...
frame.metno <- function(z=z,select = NULL, col="gray50", col.select = NULL,add.climatology=TRUE,cex.lab=1,...) {
  ...
  ## add margin text
  mtext(paste(toupper("esd package - fig-0.0 - metno 2013"),"(www.met.no)",sep=" "),side=1,line=4,cex=cex.lab)
...
if (!is.null(select)) {
   if (is.character(select)) select <- grep(tolower(select),tolower(cnames))
...
fig1.zoo <- function(z=list(z1=z1,z2=z2),select = NULL, col.select=NULL,tsline = FALSE, enveloppe=TRUE,col.enveloppe=NULL,add.clim=FALSE,add.2C=FALSE,error=-1) {

if (is.list(z)) {
   x  <- as.matrix(coredata(z[[1]])) 
   nx <- length(names(z))
   year <- as.numeric(index(z[[1]]))
} else if (is.zoo(z)) { 
   x  <- as.matrix(coredata(z)) 
   nx <- 1
   year <- as.numeric(index(z))
...
if (!is.null(select)) {
   if (is.character(select)) 
      iselect <- grep(tolower(select),tolower(cnames))
   ...
   if (length(iselect)<1) 
      ...
      for (j in 1:length(iselect)) lines(year,x[,iselect[j]],col=col.select[j],lwd=2)
      # Add legend 
      if (nx == 1)
