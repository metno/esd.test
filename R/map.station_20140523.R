...
map.station <- function (x = NULL, stid = NULL, param = NULL, lon = NULL, lat = NULL, 
                         ...
                         nmin = NULL , it = NULL ,
                         col = "darkgreen",bg="green",cex=1,breaks = NULL, zexpr = "alt",
                         select = NULL , col.select = "darkred" , bg.select = "red", cex.select=2,
                         colbar=list(breaks=NULL,type="p",cex=1,h=1,cex.lab=1),showall = FALSE,
                         verbose = FALSE , add.text=TRUE,height=NULL,width=NULL,
                         FUN=NULL,from=NULL,to=NULL,type.colbar="p",cex.lab=1,showaxis=FALSE,...) 
{ 
  ...
  if (showall==FALSE & !is.null(unlist(select))) {col = "white" ; bg="white"} 
  ...
  plot(ss$longitude, ss$latitude, pch = 21, col = "white", bg="white" ,cex = cex, xlab = "", ylab = "", xlim = xlim, ylim = ylim , axes = FALSE , frame.plot = FALSE,...)
  ...
  title(main=paste(paste(levels(factor(ss$longname)),collapse="/"),toupper(FUN),sep="/"),line=2,cex.main=.8 , adj = 0)
  ...
  if (TRUE) {  
      if (!is.null(FUN)) {
          ...
          if (is.null(colbar$breaks)) {
              ...
              colbar$breaks <- pretty(y)
          }
          COL <- rev(terrain.colors(n=length(colbar$breaks)))
          COL <- rainbow(n=length(colbar$breaks))
          ## browser()
          col.bar(colbar$breaks,col=COL,type=colbar$type,v=colbar$v,h=colbar$h,cex=colbar$cex,cex.lab=colbar$cex.lab)
 ...
 ## browser()
  ...
  if (!inherits(x,"stationmeta") & !is.null(attr(x,'na'))) scale <- attr(x,'na') else scale <- 1
  points(ss$longitude, ss$latitude, pch = 21, col = col, bg=bg ,cex = colbar$cex*scale, xlab = "", ylab = "", xlim = xlim, ylim = ylim , ...)
  ...
  if (showaxis) title(ylab = "Latitude",line=2.2 , cex.lab = cex.lab) 
                                        # format axes
  if (showaxis) axis(1,seq(xlim[1],xlim[2],by=10),cex.axis=cex) # 0.7
  if (showaxis) axis(2,seq(ylim[1],ylim[2],by=10),cex.axis=cex)
  if (showaxis) axis(4,seq(ylim[1],ylim[2],by=10),cex.axis=cex)
  if (showaxis) axis(3,seq(xlim[1],xlim[2],by=10),cex.axis=cex)
...
col.bar <- function(breaks,horiz=TRUE,v=1,h=1,col=col,cex=2,cex.lab=0.81,type="r",verbose=FALSE,vl=0.5) {
