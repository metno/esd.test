...
DSE <- function(stid="18700",cntr="NORWAY",src="METNOM",param="t2m",lon=c(-10,10),lat=c(-10,10),FUN="mean",path="CMIP5.monthly",rcp="rcp45",biascorrect=TRUE,reanalysis="ERA40_t2m_mon.nc",email=NULL,save=TRUE,force=FALSE,verbose=FALSE,out.dir="dse.test3",update=FALSE,plot=TRUE,show.map=TRUE,select=1:3,it=NULL,...) {
  ...
  if (show.map) {
    ...
    col.bar(col=col,type="p",v=6,h=6)
  ...
  if (!is.null(ss)) {
    ...
    for (i in 1:d) { 
      ...
      if (!is.null(y)) {        
        ...
        ## generate file name
        rea <-strsplit(reanalysis,split="_")[[1]][1]
        ...
        if (!file.exists(file.path(out.path,filename)) | force) {
          ## Do the downscaling
          class(ss) <- "data.frame"
            print(paste("DOWNSCALING SEASONAL",toupper(FUN),text))
          ## if (plot) par(new=TRUE)
          if (tolower(param)=='t2m') {
            z <- try(DSensemble.t2m(y,FUN=FUN,rcp=rcp,reanalysis=reanalysis,biascorrect=biascorrect,path=path,lon=lon,lat=lat,select=select,plot=FALSE,...))
            attr(z,"method") <- FUN
          } else if (tolower(param)=='precip') {
            z <- try(DSensemble.precip(y,FUN=FUN,rcp=rcp,reanalysis=reanalysis,biascorrect=biascorrect,path=path,lon=lon,lat=lat,select=select,plot=FALSE,...))
                attr(z,"method") <- FUN
          } else z <- NULL   
        }
        else {
